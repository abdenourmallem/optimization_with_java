...
public class EffFuncs {
    ...
    public static int[] sortItmsBySimpEff(MKP mkpInstance) {
        Pairs[] itmPairs = new Pairs[mkpInstance.numItems];

        for (int i = 0; i < mkpInstance.numItems; i++) {
            itmPairs[i] = calcSimpEff(i, mkpInstance);
        }

        Arrays.sort(itmPairs);

        int[] sortedItms = new int[mkpInstance.numItems];
        for (int i = 0; i < mkpInstance.numItems; i++) {
