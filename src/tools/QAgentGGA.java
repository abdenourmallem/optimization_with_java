/* 
 * Contains the Q agent implementation for the GGA algorithm
 */
package tools;
...
public class QAgentGGA {
    ...
    public static StepResult step(QAgentGGA.Action action, CandidateGGA currentState) {
        currentState.calcPosition();
        CandidateGGA newState = new CandidateGGA(currentState.mkpInstance, currentState.position);
    ...
    public void train(MKP env, CandidateGGA candidate, List<Pairs> x1, double alpha, double gamma,
            double epsilon, int eps) {
        candidate.calcPosition();
        CandidateGGA state = new CandidateGGA(env, candidate.position.clone());
    ...
    public void trainV2(MKP env, CandidateGGA candidate, List<Pairs> x1, double alpha, double gamma,
            double epsilon) {
        candidate.calcPosition();
        CandidateGGA state = new CandidateGGA(env, candidate.position.clone());
        for (int ep = 0; ep < 100; ep++) {
            ...
            CandidateGGA clone = new CandidateGGA(env, state.position.clone());
            int startIdx = 0;
    ...
    public List<Integer> qLearningLocalSearch(CandidateGGA solution, List<Pairs> x1) {

        CandidateGGA state = new CandidateGGA(solution.mkpInstance, solution.chromosome);
        state.calcPosition();
        int startIdx = x1.size();
        for (int iter = startIdx; iter < state.size; iter++) {
            ...
            Map<QAgentGGA.Action, Double> qValues = Q.get(stateKey);
            List<Map.Entry<QAgentGGA.Action, Double>> sortedActions = qValues.entrySet()
                    .stream()
                    .sorted(Map.Entry.<QAgentGGA.Action, Double>comparingByValue().reversed())
                    .collect(Collectors.toList());

            for (Map.Entry<QAgentGGA.Action, Double> entry : sortedActions) {

                QAgentGGA.Action action = entry.getKey();

                // System.out.println("Trying action: " + action.index + " " + action.operation
                // + " at iter " + i);

                StepResult result = QAgentGGA.step(action, state);
                // System.out.println("obj after step: " + state.objValue);

                if (result.done) {
                    state = new CandidateGGA(state.mkpInstance, result.newState);
                    break;
                } else {
    ...
    public static void main(String[] args) {
        ...
        CandidateGGA initialCandidate = GGA.generateIndividual(mkp, GuideParts.get(0), IntegrationRate, pb);
