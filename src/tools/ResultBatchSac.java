...
public class ResultBatchSac {
    ...
    public static class ResultRowSac {
        String instanceName;
        double successRate;
        double stDev;

        public ResultRowSac(String instanceName, double successRate, double stDev) {
    ...
    public static void saveAllResults(List<ResultRowSac> results, String filename) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
            // Write the header once
            writer.write("Instance,SR,STD\n");

            int rowsCounter = 0;
            for (ResultRowSac row : results) {
